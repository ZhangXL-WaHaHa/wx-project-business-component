    ...
    methods: {
            ...
            if (this.properties.selectDateMultiple.length !== 0) {
                ...
                this.properties.selectDateMultiple.forEach((item, index) => {
                    date.list.forEach((arr, temp) => {
                        if (
                            item ===
                            date.year + "-" + date.month + "-" + arr.value
                        ) {
                            arr.color = "select";
                        }
                    });
        ...
        formatRangeDateColor(date) {
            ...
            let beginTimeStamp = new Date(
                this.properties.selectDateRange.begin.replace(/-/g, "/")
            ).getTime();
            let endTimeStamp = new Date(
                this.properties.selectDateRange.end.replace(/-/g, "/")
            ).getTime();

            // 判断是否有选择日期
            if (
                (this.properties.selectDateRange.begin === "" &&
                ...
                date.list.forEach((item, index) => {
                    let timeStamp = new Date(
                        date.year + "/" + date.month + "/" + item.value
                    ).getTime();
        ...
        calculateResidualDays(date) {
            ...
            for (let i = 0; i < date.firstDayWeek; i++) {
                date.list.unshift({
                    value: this.properties.showRemnantDays
                        ? last_value - i
                        : "",
                    type: "last",
                });
        ...
        getLastMonth() {
            let changeKey =
                DAY_TYPE[
                    this.data.showCalendarIndex - 1 >= 0
                        ? this.data.showCalendarIndex - 1
                        : 2
                ];
            let date = this.data.calendarInfo[
                DAY_TYPE[this.data.showCalendarIndex]
            ];
        ...
        selectDate(e) {
            ...
            if (e.currentTarget.dataset.type === "last") {
                ...
                // 单一模式下选择时间节点
                this.properties.selectDate =
                    date.year + "-" + date.month + "-" + date.list[index].value;
                // 重置三个月之内选中节点
        ...
        selectDateRange(key, index) {
            ...
            if (this.properties.selectDateRange.begin === "") {
                ...
                this.setData({
                    [`calendarInfo.${key}.list[${index}].color`]: "select",
                    [`calendarInfo.${key}.list[${index}].rangTip`]: this
                        .properties.selectDateRange.beginText,
                });
            } else if (
                this.properties.selectDateRange.begin !== "" &&
                this.properties.selectDateRange.end !== ""
                ...
                this.data.calendarInfo[key].list[
                    index
                ].rangTip = this.properties.selectDateRange.beginText;
                this.setData({
                ...
                if (beginTimeStamp > endTimeStamp) {
                    ...
                    this.properties.selectDateRange = {
                        ...
                        begin:
                            ...
                            date.month +
                            "-" +
                            date.list[index].value,
                        end: "",
                    };
                    Object.keys(this.data.calendarInfo).forEach((item) => {
                        this.data.calendarInfo[item].list.forEach(
                            (arr, len) => {
                                if (arr.color === "select") {
                                    arr.color = "unSelect";
                                    delete arr.rangTip;
                                    this.setData({
                                        [`calendarInfo.${item}.list[${len}]`]: this
                                            .data.calendarInfo[item].list[len],
                                        [`calendarInfo.${key}.list[${index}].color`]: "select",
                                        [`calendarInfo.${key}.list[${index}].rangTip`]: this
                                            .properties.selectDateRange
                                            .beginText,
                                    });
                                }
                            }
                ...
                if (this.properties.selectDateRangeSize >= dayRange) {
                    ...
                    if (differDay > this.properties.selectDateRangeSize) {
                        wx.showToast({
                            title:
                                "选择的天数不能超过" +
                                this.properties.selectDateRangeSize +
                                "天",
                            icon: "none",
                        });
                        // 计算结束的位置
                        let differTimeStamp =
                            (this.properties.selectDateRangeSize - 1) *
                            24 *
                            60 *
                            60 *
                            1000;
                ...
                Object.keys(this.data.calendarInfo).forEach((item) => {
                    this.data.calendarInfo[item].list.forEach((arr, len) => {
                        let timeStamp = new Date(
                            this.data.calendarInfo[item].year +
                                "/" +
                                this.data.calendarInfo[item].month +
                                "/" +
                                arr.value
                        ).getTime();
                        if (
        ...
        selectDateMultiple(key, index) {
            ...
            if (date.list[index].color === "select") {
                ...
                // 直接添加对应的数据
                this.properties.selectDateMultiple.push(
                    date.year + "-" + date.month + "-" + date.list[index].value
                );
                this.setData({
                    [`calendarInfo.${key}.list[${index}].color`]: "select",
            ...
            holiday.holidayList.forEach((item, index) => {
                date.list.forEach((arr, temp) => {
                    ...
                    switch (arr.type) {
                        ...
                        case "next":
                            value =
                                (date.month + 1 > 12 ? 1 : date.month + 1) +
                                "-" +
                                arr.value;
                            break;
                    }
        ...
        // 点击显示下一月
        showNextMonth() {
            preHeight = this.data.calendarInfo[
                DAY_TYPE[this.data.showCalendarIndex]
            ].swiperHeight;
        ...
        showLastMonth() {
            ...
            this.setData(
                {
                    showCalendarIndex:
                        --this.data.showCalendarIndex < 0
                            ? 2
                            : this.data.showCalendarIndex,
                },
                () => {
                    this.calendarHeightTransition();
                }
        ...
        tapBtn() {
            ...
            switch (this.properties.calendarType) {
                case "multiple":
                    this.triggerEvent(
                        "finishSelectDate",
                        this.properties.selectDateMultiple
                    );
                    break;
                case "range":
                    this.triggerEvent(
