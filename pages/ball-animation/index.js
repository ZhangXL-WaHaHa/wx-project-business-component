...
Page({
    ...
    onLoad: function () {
        ...
        for (let i = 0; i < 20; i++) {
            this.data.commodityList.push({
                image:
                    "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1598635719606&di=d90965331efd5c6dc2645ecb5fbba324&imgtype=0&src=http%3A%2F%2Fimg3.imgtn.bdimg.com%2Fit%2Fu%3D2905120353%2C3638949522%26fm%3D214%26gp%3D0.jpg",
                name: "这是商品",
                price: 99,
            });
        }
    ...
    areAppMessage: function () {
        ...
        return {
            ...
            path:
                "/pages/orderdetail/orderdetail?shopId=" +
                _that.data.mechine.shopId +
                "&id=" +
                _that.data.mechine.id +
                "&address=" +
                _that.data.mechine.address,
        };
    },
    //右侧列表滚动事件
    ...
    getIndexFromHArr: function (value) {
        ...
        for (var j = 0; j < this.data.goodsNumArr.length; j++) {
            if (
                value >= this.data.goodsNumArr[j] &&
                value < this.data.goodsNumArr[j + 1]
            ) {
    ...
    _resetTotalNum: function () {
        ...
        if (shoppingCartGoodsId) {
            for (let i = 0; i < shoppingCartGoodsId.length; i++) {
                let goodNum = Number(
                    this.data.shoppingCart[shoppingCartGoodsId[i]]
                );
                totalNum += Number(goodNum);
                totalPay +=
                    Number(this.data.goodMap[shoppingCartGoodsId[i]].price) *
                    goodNum;
    ...
    goPayFn: function (e) {
        ...
        for (let i = 0; i < this.data.shoppingCartGoodsId.length; i++) {
            ...
            quantitys +=
                this.data.shoppingCart[this.data.shoppingCartGoodsId[i]] + ",";
        }
